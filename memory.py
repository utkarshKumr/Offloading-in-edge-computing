...
class MemoryDNN:
    ...
    def plot_cost(self, net_num):
        import matplotlib.pyplot as plty
        plty.figure(net_num)
        colors ="bgrcmykw"
        for p in range(self.net_num):
            plty.plot(np.arange(len(self.cost_his[p])), self.cost_his[p],colors[np.random.randint(0,8)])
        plty.ylabel('Cost of MemoryDNN')
        plty.xlabel('training steps')
        # plty.show()
        plty.savefig("plotcost_" + str(net_num) + ".png")
